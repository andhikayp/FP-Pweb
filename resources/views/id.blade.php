                ...
                @foreach ($posts as $post)
                    <div class="card">
                        ...
                        <div class="card-header">
                            ...
                            <a class="font-weight-bold" href="{{ route('post.show', $post->id)}}">{{ $post->title}} </a> <br>{{-- {{ $post->created_at->diffForHumans()}} --}} {{ date('F d, Y',strtotime($post->created_at)) }} at {{ date('g:ia',strtotime($post->created_at)) }} by <a href="{{ route('home.id', $post->user_id)}}"> {{ $sembarang->username}}</a> <br><br>
